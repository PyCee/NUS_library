...
NUS_vector nus_vector_scale(NUS_vector NUS_vector_0, double scale)
{
  return nus_vector_build(NUS_vector_0.x * scale,
			  NUS_vector_0.y * scale,
			  NUS_vector_0.z * scale);
}
...
{
  double mag_sq = nus_vector_mag_sq(NUS_vector_);
  if(0.0 == mag_sq){
    return NUS_vector_;
  }
...
(NUS_vector NUS_vector_1, NUS_vector NUS_vector_2, double t)
{
  double b;
  t = (t < 1.0) ? t : 1.0;
  t = (t > 0.0) ? t : 0.0;
  b = 1.0 - t;
  return nus_vector_add(nus_vector_scale(NUS_vector_1, b),
			nus_vector_scale(NUS_vector_2, t));
}
double nus_vector_mag_sq(NUS_vector NUS_vector_)
{
  return nus_vector_dot(NUS_vector_, NUS_vector_);
}
char nus_vector_cmp(NUS_vector NUS_vector_0, NUS_vector NUS_vector_1, double range)
{
  return nus_vector_mag_sq(nus_vector_subtract(NUS_vector_0, NUS_vector_1)) <=
    range * range;
}
