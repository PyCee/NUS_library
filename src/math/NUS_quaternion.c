...
{
  return (NUS_quaternion){.w = w, .x = x, .y = y, .z = z};
}
...
NUS_quaternion nus_quaternion_unit(NUS_vector vector, double radians)
{
  ...
  return nus_quaternion_build(cos(half_radians),
			      sin_half_radians * vector.x,
...
{
  return nus_quaternion_build(/* h product w component */
